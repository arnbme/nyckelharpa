 ...
 *	Mar 05, 2019 V2.1.4  **Not done Added**: Boolean flag for debug message logging, default false
 *	Jan 06, 2019 V2.1.3H Added: Support for 3400_G Centralite V3
 ...
 */
definition(
    name: "Nyckelharpa Contact",
    namespace: "arnbme",
    author: "Arn Burkhoff",
    description: "(${version()}) Nyckelharpa Contact Sensor Profile",
    category: "My Apps",
...
def pageZero()
	...
	dynamicPage(name: "pageZero", title: "This App cannot be installed", uninstall: true, install:false)
		...
		section
			{
			paragraph "This SmartApp, SNyckelharpa Contact, cannot be installed. Please install and use SHM Delay."
			}
		}
	}	
...
*/		if (thecontact.typeName.matches("(.*)(?i)(keypad)(.*)"))
		...
		if ((thecontact.typeName.matches("(.*)(?i)simulated(.*)") ||
			thecontact?.data.manufacturer == null && thecontact?.data.model==null &&
...
/*	if (thesimcontact)
		...
		if (thesimcontact.typeName.matches("(.*)(?i)simulated(.*)") ||
			(thesimcontact?.data.manufacturer == null && thesimcontact?.data.model==null &&
...
def doNotifications(message)
	...
	if (theLog)
		{
		parent.sendNotificationEvent(localmsg)
		}
	if (thesendPush && parent.sendPushMessage)
		{
		parent.sendPushMessage.deviceNotification(localmsg)
		}
	if (phone)
		{
...
// shows as text 			msg_emergency= "<a href=\"tel://${parent?.global911} \">${parent?.global911}</a>"
				...
				{
				message+=" detected (Nyckelharpa App)"
				}
...
//	def alarmvalue = alarmstate.value
	def alarmvalue = location.hsmStatus	//get HE alarm status
...
//	if ((alarmvalue == "stay" || alarmvalue == "away") && door_count>0)
	if (alarmvalue.substring(0,5) == "armed" && door_count>0)
		{
...
def checkOtherDelayProfiles(baseContact, baseMotion, baseEntryDelay)
	...
	def profiles=parent.findAllChildAppsByName('Nyckelharpa Contact')
