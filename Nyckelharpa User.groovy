 ...
 */
definition(
    name: "Nyckelharpa User",
    namespace: "arnbme",
    author: "Arn Burkhoff",
    description: "(${version()})  Maintain User Pins for Nyckelharpa.",
    category: "My Apps",
...
def pageZero()
	...
	dynamicPage(name: "pageZero", title: "This App cannot be installed", uninstall: true, install:false)
		...
		section
			{
			paragraph "This SmartApp, Nyckelharpa User, must be used as a child app of Nyckelharpa."
			}
		}
	}	
...
def isunique()
	...
	children.each
		{ child ->
		if (child.getName()=="Nyckelharpa User")		//process only pin profiles	
			{
//			verify unique name
...
//	use only the time portion HH:MM when comparing and testing for when pin is entered in Nyckelharpa
	...
	dynamicPage(name: 'pageFour', title: 'Pin Msg Overrides') 
		...
		section
			...
			input "UserPinPhone", "phone", required: false, 
				title: "Send Pin text message to this number. For multiple SMS recipients, separate phone numbers with a semicolon(;)or poundsign(#)"
			}
		}
	}
